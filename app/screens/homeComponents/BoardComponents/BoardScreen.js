...
export default class BoardScreen extends Component {
  ...
  timeBefore(date) {
    ...
    console.log(date)
    if(now.getFullYear() > date.slice(0,4)){
        minus= now.getFullYear()-date.slice(0,4);
  
        console.log(minus+"년 전");
        return (minus+"년 전")
    }else if(now.getMonth() > date.slice(5,7)){
  
        minus= now.getMonth()-date.slice(5,7);
        return (minus+"달 전")
    }else if(now.getDate() > date.slice(8,10)){

        minus= now.getDate()-date.slice(8,10);
        return (minus+"일 전")
    }else if(now.getDate() == date.slice(8,10)){
 
        var nowTime = now.getTime();
        var date = date.slice(11,19);
        if(nowTime>date){
    ...
    render(){
      ...
      <View style = {styles.item}> 
        <TouchableOpacity style = {{width:'90%'}} onPress = {()=>this.props.navigation.navigate("contentscreen",{idx:item.idx})}>
          <Text style={styles.writes}>{item.title}</Text>
          <Text>{this.timeBefore(item.created)} 작성자: {item.writer}</Text>                      
          </TouchableOpacity>
        {user_id == item.id &&  <View style={{flex:1,alignItems:"flex-end" ,justifyContent:"flex-end"}}>
